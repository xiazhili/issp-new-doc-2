...
//        }
        entity.setConditions(to.getType());
...
//        entity.setCreator(userAPI.currentUser().getUsername()); todo测试注释
    ...
    public void sendEmail(CollectEmailTO to) throws SerException {
        String userToken = RpcTransmit.getUserToken();
        /*if (StringUtils.isBlank(CollectEmailTO.getEmpId())) {
            throw new SerException("id不能为空");
        }*/
       /* if (StringUtils.isNotBlank(to.getAccount()) && !Validator.isEmail(CollectEmailTO.getAccount().trim())) {
            throw new SerException("邮箱书写不正确");
        }
...
//                List<NotificationFormulaBO> signBOList = collectEmailSer.collectCollectEmail(condis);
                List<BusinessContractCollectBO> bos = businessContractProSer.collectCollectEmail(sign.getType());
                //拼表格
                String content = getBCCTable(bos);

                MessageTO messageTO = new MessageTO();
                messageTO.setContent( content );
                messageTO.setTitle("定时发送项目实施进度管理汇总汇总");
                messageTO.setMsgType(MsgType.SYS);
                messageTO.setSendType( SendType.EMAIL);
                messageTO.setRangeType( RangeType.SPECIFIED);
                //定时发送必须写
    ...
    private String getBCCTable(List<BusinessContractCollectBO> list) {
        ...
        if (list != null && list.size() > 0) {
            sb = new StringBuffer("<h4>项目实施进度管理汇总汇总:</h4>");
